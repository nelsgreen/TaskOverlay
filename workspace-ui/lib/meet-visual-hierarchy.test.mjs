...
// warm-graphite card/text softening
// ---------------------------------------------------------------------------
...
test("the proposal card is Dark-only warmed off pure --surface-sunken (a color-mix toward --warning), not a global Dark token change - Light keeps bg-surface-sunken unchanged", () => {
  assert.match(meetingAssistantSrc, /bg-surface-sunken p-2 dark:bg-\[color-mix\(in_oklch,var\(--surface-sunken\)_92%,var\(--warning\)_8%\)\]/)
})

test("the proposed title (primary card text) is Dark-only softened off pure --text, and rationale/quote (secondary text) are Dark-only warm-muted with improved size/line-height in both themes", () => {
  const cardBlock = proposedActionCardBlock()
  assert.match(cardBlock, /dark:text-\[color-mix\(in_oklch,var\(--text\)_85%,var\(--warning\)_15%\)\]/)
  assert.equal((cardBlock.match(/text-\[11px\] italic leading-relaxed text-muted-foreground dark:text-\[color-mix\(in_oklch,var\(--text-muted\)_85%,var\(--warning\)_15%\)\]/g) ?? []).length, 1)
  assert.equal((cardBlock.match(/text-\[11px\] leading-relaxed text-muted-foreground dark:text-\[color-mix\(in_oklch,var\(--text-muted\)_85%,var\(--warning\)_15%\)\]/g) ?? []).length, 1)
